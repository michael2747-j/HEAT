...
void ExportFirewallLog(const std::string& path, std::ofstream& log, std::ofstream& summary) {
    ...
    while (std::getline(fwlog, line)) {
        ...
        if (!line.empty() && line[0] != '#') {
            // Example: Just print the action and IPs
...
void WriteEventIdReference(const std::string& filename) {
    std::ofstream ref(filename, std::ios::out | std::ios::trunc);
    if (!ref.is_open()) {
    ...
    ref << "----------------------\n";
    for (int id : encounteredEventIds) {
        ref << "EventID: " << id << " - ";
...
int main() {
    ...
    std::cout << "Logs exported to historical_network_log.txt, summary_network_log.txt, and event_id_reference.txt" << std::endl;
    return 0;
}
